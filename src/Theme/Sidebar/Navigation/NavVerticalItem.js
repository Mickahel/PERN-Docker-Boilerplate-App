...
function NavVerticalItem(props) {
    ...
    const navBarCloseMobile = () => { themeContext.setSidebarOpen(false)}
    ...
    return <ListItem
        ...
        onClick={(event) => {
            ...
            if(matches || themeContext.sidebarOpenedEvent==="click") navBarCloseMobile()
        }}
    >
        <ListItemIcon
            ...
            >
            <span className={classnames('list-item-icon text-16 flex-no-shrink mr-2',checkIfActive() && classes.activeColor)} >
                {item.icon}
            </span>
        </ListItemIcon>
