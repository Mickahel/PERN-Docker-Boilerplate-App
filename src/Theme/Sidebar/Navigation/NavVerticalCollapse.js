...
let isUrlInChildren = (parent, url) => {
    ...
    for (let i = 0; i < parent.children.length; i++) {
        if (parent.children[i].children && isUrlInChildren(parent.children[i], url)) return true;
        if (parent.children[i].to === url || (!parent.children[i].exact && url.includes(parent.children[i].to))) return true;
    }
    return false;
}
function NavVerticalCollapse(props) {
    ...
    let needsToBeOpened =useCallback( (item) => history.location && isUrlInChildren(item, history.location.pathname), [])
    ...
    return (
        <div className={classNames((open && "open"))}>
            ...
            >
                <ListItemIcon
                    classes={{ root: styledClasses.listItemIcon }}
                >
                ...
                <Icon className={classNames("w-32  p-0 flex", styledClasses.bigCollapseIcon, {
                    [styledClasses.bigCollapseIconShift]: themeContext.sidebarOpen
                })}>
                    {open ? <ExpandLessOutlinedIcon color="action" /> : <ExpandMoreOutlinedIcon color="action" />}
                </Icon>
