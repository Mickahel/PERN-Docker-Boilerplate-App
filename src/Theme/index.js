...
export default function Theme(props) {
    ...
    const isOpen = (value, openType) =>{
        if (themeContext.sidebarOpenedEvent==="click" && openType=="hover" && themeContext.sidebarOpen===true) return 
        themeContext.setSidebarOpen(value)
        themeContext.setSidebarOpenedEvent(openType)
    }
    return (
        <div className="flex h-full">
            {config.theme.header.enabled && <Header
                handleDrawerOpenOnClick={handleDrawerOpenOnClick}
            />}
            {themeContext.showSidebarComponents(matches) && <Sidebar
