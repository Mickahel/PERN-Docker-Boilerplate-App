...
function StandardDialog(props){
  ...
  const createButtons = (formikBag, showMoreInfo) =>{
    ...
    if(Array.isArray(additionalButtons)){
      for(let i = 0; i<additionalButtons.length; i++){
        ...
        buttons.push(<Button key= {single.text} className={classNames(single.className)} onClick={single.onClick} disabled={single.disabled}>
          {single.text}
          </Button>)
      }
    }
    if(showMoreInfo){
      buttons.push(<Button  key='2' className="more-info" onClick={()=>setMoreOpen(!moreOpen)}>
          {moreOpen ? "Show less" : "More info"}
          </Button>)
    }

    if(type === 'confirm'){
      buttons.push(<Button  key='3' className='close' onClick={hideDialog(formikBag)}>
        <Trans>cancel</Trans>
      </Button>)
      buttons.push(<Button  key='4' className='ok' type='submit'><Trans>ok</Trans></Button>)
    }else{ // Success, error, ....
      buttons.push(<Button  key='5' className='close' onClick={hideDialog(formikBag)}>
        <Trans>close</Trans>
      </Button>)
    }
  ...
  const { open, type, icon, modal, inputIcon, inputlabel,err } = themeContext.dialog
  let { title, message, moreInfo } = themeContext.dialog

  let showMoreInfo = Boolean(moreInfo)
  //-------------------------- Error handling -------------------------
  if(err){ // qui ho un errore
  ...
  //------------------------- Icon -------------------------------------
  let realIcon = null
  if (!icon) { // Switch su type
    switch(type){
        case "error":
          realIcon = <ErrorOutlineOutlinedIcon  className="iconTitleColor" />
        break;
        case "success":
          realIcon = <CheckCircleOutlineOutlinedIcon  className="iconTitleColor" />
        break;
        case "info": 
        case "confirm":
        ...
        return <Form>
          ...
          >
            ...
            <DialogContent className='content'>
              <DialogContentText className='content-text'> {message} </DialogContentText>
              {type === 'confirm' &&
