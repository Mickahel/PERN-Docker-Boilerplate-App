...
const EnhancedTableToolbar = (props) => {
  const { numSelected,handleEditClick,handleDeleteClick,handleSearch,headCells,handleCheckboxFilterClick,showFilters,showSearchbar,readOnly } = props;
      ...
      >
        {numSelected > 0 ? (
          <Typography className={classes.title} color="inherit" variant="subtitle1" component="div">
            {numSelected} {numSelected===1  ? <Trans>enhancedTable.elementSelected</Trans> : <Trans>enhancedTable.elementsSelected</Trans>}
          </Typography>
        ) : (
          <>
            {showSearchbar && <TextField size="small" onChange={(e)=>{handleSearch(e)}} label={<Trans>enhancedTable.search</Trans>} />}
          </>
        )}
        ...
        {(numSelected ===0 && showFilters) && 
          ...
          <Menu
            ...
            {headCells.map((element, index)=>{
              ...
              return (
              <MenuItem key={"check" + element.id +index} onClick={(e)=>handleCheckboxFilterClick(e, element.id)}>
                <Checkbox
                  color="primary"
